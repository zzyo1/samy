...
window.scanIps = async function(ips, conf, subnet)
{
  ...
  for (let i = 0; i < ips.length; i += conf.block)
    liveIps = Object.assign(liveIps, await window.scanIpsBlock(ips.slice(i, i+conf.block), conf, subnet))
...
window.scanIpsBlock = async function(ips, conf, subnet)
{
  ...
  let addLive = async function(lip, time)
  {
    ...
    if (conf.rtc !== false)
      await window.connectPeers(lip, function(tip)
      {
        if (conf.logger)
  ...
  if (subnet)
    for (let net of Object.keys(liveIps))
    {
      ...
      Object.assign(liveIps, await window.scanIps(window.unroll_ips(getSubnet(net)+'*', 1, 254), conf))
    }
...
window.webScanAll = async function(nets, conf)
{
  ...
  ips.network = await window.scanIps(window.unroll_ips(nets), conf, true)
